...
| 工具 | 用途 | 步骤 |
|------|------|------|
| `tools/cnipa_epub_search.py` | Playwright 浏览器自动化 → 国知局公布公告站查新 | Phase 2 3 |
| `prompts/disclosure_builder.md` | 脱敏映射规则 | Phase 5 本地项目脱敏 |
| `prompts/prior_art_search.md` | 国知局查新搜索策略参考 | Phase 2 3 |

**查新工具依赖安装(首次使用前):**
```bash
pip install -r skills/patent-workflow/tools/requirements-cnipa.txt
python -m playwright install chromium
```

**查新工具用法:**
```bash
# 每轮只传一个检索词;2-8 个检索单位需 2-8 次独立调用
python skills/patent-workflow/tools/cnipa_epub_search.py <检索词>
# stdout 仅一行 EPUB_HITS_JSON: [...] - 含标题、公开号、abstract
```
...
- 当 Phase 2 证据中存在 `source_type=local_project` 或 manifest 存在 `local_project_paths` 时,正文写入后必须执行本地材料脱敏过滤,并保存 `artifacts/draft/redaction_policy.json`;脱敏规则读取 `prompts/disclosure_builder.md` 的脱敏映射表:公司/产品名泛化为"某系统",客户/项目标签泛化为"对象A",内部路径泛化为"本地路径",真实分类标签泛化为"分类A",具体业务规模泛化为"一定规模"。联网搜索-only 流程不得启用该过滤。
...
- 复用自检闭环思想,新增 `technical_closure` 检查:正文必须形成输入/采集→处理/判断→输出/记录/反馈闭环,并说明异常、边界样本、低置信度或冲突分支处理路径。
...
## Phase 2 Agent-Native 执行指南(v5 - 2026-05-25 更新)
...
查新使用本 skill 内置的 `tools/cnipa_epub_search.py`(Playwright 浏览器自动化),优先国知局公布公告站;翻车时降级 Google Patents。
...
║              Phase 2 执行顺序(不可跳步)                    ║
║                                                          ║
║  ☐ 1 领域搜索 → 产出「初版创新点」(rough ideas, 非候选)     ║
...
║  ⚠️ 没跑查新之前不得生成候选专利名称                          ║
║  ⚠️ 查新优先国知局Playwright → 翻车才降级Google Patents        ║
║  ⚠️ 不得仅用LLM生成摘要当查新结果 - 必须打开专利页面核实        ║
╚══════════════════════════════════════════════════════════╝
```
...
| 步骤 | 叫什么 | 查什么 | 不查什么 | 时机 |
|------|--------|--------|----------|------|
| 2 专利复核 | 验证引证专利真实性 | 专利号是否存在、标题摘要是否匹配、法律状态是否有效 | ❌学术论文 ❌行业报告 ❌社会热点 | 领域搜索后、查新前 |
| 3 国知局查新 | 创新点新颖性检索 | 初版创新点附近有无冲突的已授权/在审专利 | ❌验证非专利真伪 | 复核后、生成候选前 |
| 非专利筛查 | 相关性与时效检查 | 是否与方向相关、是否在18个月内 | ❌验证真伪(公开网页无需) | 贯穿 Phase 2 |

### 前置条件

Agent 在执行 Phase 2 前必须确认:
1. Phase 0 预处理已完成(`artifacts/run_manifest.json` 存在,phase 0 status=success)
2. Phase 1 范围已确认(manifest 中有 `domain_scope`、`phase_02_mode`、`local_project_inquiry_done=true`)
...
2. **参考专利线索**:smart-search 发现的相近专利号(这些专利号尚未验证,需在 2 复核)
...
#### Step 4.5: 2 专利复核 -- 验证引证专利真实性

⚠️ **初版创新点产出后、查新前必须执行。**

对领域搜索中发现的每个专利号,用 `smart-search fetch` 抓取实际页面,确认:

| 验证项 | 方法 |
|--------|------|
| 专利号是否存在 | `smart-search fetch "https://patents.google.com/patent/CNXXXXXXXXXA/zh"` 查看实际标题 |
| 标题是否匹配 | 对比页面实际标题与搜索结果中的声称标题 |
| 法律状态 | 页面中 legal status 字段:授权/实审中可用,撤回/驳回/失效排除 |

**禁止**仅凭 LLM 搜索结果中的摘要判断专利真伪。GLM5 曾将土壤健康专利包装为座舱安全专利。

复核后筛选出「真实且有效的专利号」供 3 查新和 4 候选生成引用。

---

#### Step 5: 3 国知局查新 -- 创新点新颖性检索 (⚠️ 硬性前置步骤)
...
```
1 国知局 Playwright 自动化(优先)
   └─ 成功 → 产出 phase_02_patent_verification.json
   └─ 翻车 → 降级 2
2 Google Patents 页面抓取(降级)
```

**翻车判定**(满足任一即降级):命令非 0 退出、超时、`EPUB_HITS_JSON` 为空数组、条目明显与主题无关。

---

##### 1 国知局 Playwright 自动化(优先路径)

使用本 skill 内置的 `tools/cnipa_epub_search.py`(Playwright 浏览器自动化),去国知局公布公告站 (epub.cnipa.gov.cn) 检索。
...
1. **生成检索词**:从初版创新点中提取 2-8 个有检索意义的语义单位(专业术语、名词短语、业内固定搭配),不要拆成过碎的单字或泛义双字

2. **逐词检索国知局**(每轮 Bash 只传一个检索词,2-8 次独立调用):
   ```bash
   python skills/patent-workflow/tools/cnipa_epub_search.py <检索词1>
   python skills/patent-workflow/tools/cnipa_epub_search.py <检索词2>
   # ... 共 2-8 次
   ```
   每次调用 stdout 仅一行 `EPUB_HITS_JSON:` + JSON 数组(含标题、公开号、abstract、详情链接等)

3. **合并去重**:按 `pub_number` 为主键去重合并所有轮次的 JSON 结果

4. **`abstract` 字段必用**:若某条含非空的 `abstract`(国知局公布站摘要),对该条专利的技术方案概括、应用场景与局限性分析,必须先基于对 abstract 的完整阅读理解后再撰写,禁止仅凭标题或公开号臆造

##### 2 Google Patents 降级路径

国知局翻车时启用。用 `smart-search fetch` 或 `web_fetch` 抓取 Google Patents 页面:

```bash
# 关键词搜索
web_fetch "https://patents.google.com/?q=<关键词>&language=ZH&status=GRANT"
# 或精准专利号验证
smart-search fetch "https://patents.google.com/patent/CNXXXXXXXXXA/zh"
```

同样需要从实际页面提取标题、摘要、法律状态,不可仅依赖 LLM 摘要。
...
```json
{
  ...
  "search_tool": "cnipa_epub_search.py (Playwright)",
  "verification_records": [
    {
      "patent_id": "CNXXXXXXXXXA",
      "verified_title": "从国知局 EPUB_HITS_JSON 查到的实际标题",
      "verified_abstract": "从国知局 abstract 字段获取的实际摘要,不少于 50 字符",
      "verification_source": "CNIPA epub.cnipa.gov.cn",
      "verification_url": "国知局详情链接(从 JSON link 字段)",
      "verification_date": "YYYY-MM-DD",
      "legal_status": "从详情页或 Google Patents 补充核实",
      "publication_date": "YYYY-MM-DD",
      "matched_in_direction": "被哪个候选方向引用",
      "description_match_note": "基于 abstract 理解后撰写的关系说明"
    }
  ]
}
```
...
| 字段 | 来源 | 用途 |
|------|------|------|
| `abstract` | 国知局 `EPUB_HITS_JSON` 的 `abstract` 字段 | **必须基于此理解专利技术方案**,禁止仅凭标题概括 |
| `legal_status` | 国知局详情页 / Google Patents 补充 | 排除撤回/驳回/失效 |
...
#### Step 6: 基于查新结果 → 优化改良点 → 生成候选专利推荐

**查新后不得直接生成候选,必须先做改良点优化。**

##### 6a. 自动优化改良点

根据查新结果分类,对每个初版创新点做差异化调整:

| 查新结果 | 处理方式 |
|----------|---------|
| 🟢 无冲突 | 保留原方向,在改良点中明确标注"查新未发现直接冲突" |
| 🟡 有重叠 | **必须调整改良点**:标注与相近专利的差异化方向,写明"本方案侧重X,而已有专利侧重Y" |
| 🔴 严重撞车 | 放弃该方向,在 manifest 中记录 `direction_abandoned` + 原因 |

优化时必须基于查新中实际读到的 `abstract` 内容,**禁止**仅凭标题做差异化判断。
...
##### 1 `artifacts/research/phase_02_research_pack.json`
```json
{
...
##### 2 `artifacts/prior_art/phase_02_patent_candidate_pool.json`
```json
{
...
##### 3 `artifacts/prior_art/phase_02_evidence_pack.json`
```json
{
