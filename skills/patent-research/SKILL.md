...
8. vault 存在时（`~/.patent-vault/` 已初始化）：方向收敛后，把**未被选中**的候选方向逐条列出，经用户确认后入方向池——先复制本轮 research pack 为 `<vault 根>/research_snapshots/<run_id>.json`，再 `vault.py add-direction --json-file <临时json>`，direction 的 `source_run.research_snapshot` 填**相对 vault 根**的 `research_snapshots/<run_id>.json`（细则见 patent-vault）。vault 未初始化且本轮未问过（无 manifest 或 manifest 无 `vault_opted_out`）→ 按 patent-vault「未初始化引导」问一次：同意即 init 后照常入池，拒绝则跳过本条。
